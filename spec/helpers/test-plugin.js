...
class TestPlugin {
    ...
    /**
     * "augment" the delta generated by UOT plugin by adding the absolute times
     * corresponding to the intervals-from-now.  These intervals are rounded to nearest second
     * and progress is hard to measure when running tests with sub-second heartbeat.
     *
     * @memberof TestPlugin
     */
    doFixups() {
        const device_readings = this.plugin.getHandler(this.deviceName).readings;
        device_readings.oldDeltaValues = device_readings.deltaValues;
        device_readings.deltaValues = function () {
            const delta = this.oldDeltaValues();

            delta._ms_statusStart = this.edgeDate;
            delta._ms_historyStart = this.historyDate;
            delta._ms_lastCycleStart = this.cycles.get(this.cycles.size()-1).date;
            delta._ms_moment = Date.now();

            // verify the offical xxxStart stats are really correctly rounded from the absolute times they're based on.
            // values are rounded to nearest second (+/- 500ms), but we allow an extra 10ms in case there's a delay
            // from .oldDeltaValues() to this call.
            expect(MATH.abs(delta._ms_moment - 1000 * delta.statusStart - this.edgeDate)).toBeLessThanOrEqual(510);
            expect(MATH.abs(delta._ms_moment - 1000 * delta.historyStart - this.historyDate)).toBeLessThanOrEqual(510);
            expect(MATH.abs(delta._ms_moment - 1000 * delta.lastCycleStart - this._ms_lastCycleStart)).toBeLessThanOrEqual(510);
...
async function newTestPlugin() {
    ...
    tp.doFixups();          // then swoop in for the kill
    return tp;
}
...
module.exports = {
    TestPlugin, delay, newTestPlugin
};
